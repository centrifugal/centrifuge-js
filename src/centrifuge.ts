...
export class Centrifuge extends (EventEmitter as new () => TypedEventEmitter<ClientEvents>) {
  ...
  async presence(channel: string): Promise<PresenceResult> {
    ...
    await this._methodCall();
    const reply = await this._callPromise(cmd, (reply: any) => reply.presence);

    const clients = reply.presence;
    const isProtobuf = this._codecName() === 'protobuf';

    for (const clientId in clients) {
      if (Object.prototype.hasOwnProperty.call(clients, clientId)) {
        const rawClient = clients[clientId];
        const connInfo = isProtobuf ? rawClient['connInfo'] : rawClient['conn_info'];
        const chanInfo = isProtobuf ? rawClient['chanInfo'] : rawClient['chan_info'];
        if (connInfo) {
  ...
  async presenceStats(channel: string): Promise<PresenceStatsResult> {
    ...
    if (isProtobuf) {
      cmd['presenceStats'] = { channel };
    } else {
      cmd['presence_stats'] = { channel };
    }

    await this._methodCall();

    const reply = await this._callPromise(cmd, (reply: any) => {
      return isProtobuf ? reply.presenceStats : reply.presence_stats;
    });

    return {
      numUsers: isProtobuf ? reply.numUsers : reply.num_users,
      numClients: isProtobuf ? reply.numClients : reply.num_clients
    };
  }
